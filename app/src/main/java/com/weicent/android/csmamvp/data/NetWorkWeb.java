...
public class NetWorkWeb<T extends Object> {
    ...
    private AsyncHttpClient mRequestClient;
    ...
    public AsyncHttpClient getHttpClient() {
        ...
        PersistentCookieStore mCookieStore = new PersistentCookieStore(MyApplication.getContext());
        client.setCookieStore(mCookieStore);
        client.setTimeout(Constants.NET_OUT_TIME);
        return client;
    }
...
//        getHttpClient().post(App.getContext(), request, stringEntity, RequestParams.APPLICATION_JSON, handler);
        mRequestClient= getHttpClient();
        mRequestClient.post(MyApplication.getContext(), request, stringEntity, RequestParams.APPLICATION_JSON, handler);
    }
    ...
    public void doRequest(String aName, ResultHandlerForJson<T> handler, String requestStr, int aType) {
        postJson(Constants.BASE_URL + aName + "?action=" + aType,requestStr,handler);
    }

    //请求操作
    public void doRequest(String aName, ResultHandlerForJson<T> handler, RequestParams  requestParams) {
        post(Constants.BASE_URL + aName,handler,requestParams);
    }
